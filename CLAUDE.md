...
| `lead.js` | serverless | **Lead-opvang** (contactform + advertentie-landingspagina + 2-staps mockup-intake): schrijft naar Supabase `stolkwebdesign_client_projects` (status `nieuwe_lead`) + **Telegram-seintje**. Sinds 10-07 stuurt óók de stap-2 mockup-intake (`mode:'details'`) een eigen Telegram-bericht (bedrijf/uitstraling/doel/USP/referenties), die kwam eerder alleen op de lead-kaart. Anti-spam honeypot/time-trap/rate-limit. **Vervangt de oude Notion-route** (zie Advertentie-funnel). **Speed-to-lead sinds 21-08:** na de insert gaat er direct een huisstijl-bevestigingsmail naar de lead via Resend (afzender peter@stolkwebdesign.nl, belofte "reactie binnen één werkdag"): vaste tekst met één AI-parafrase van het bericht ("Ik lees dat je …", claude-haiku, 3s-timeout, bedragen/budget verboden, valt terug op een generieke zin). Mailstatus staat als regel in het Telegram-seintje; een mail-fout breekt de lead nooit. Livetest 21-08: mail binnen ~2s na de insert in de inbox, parafrase klopte. **Let op dubbelmail:** automation-flow "Nieuwe lead opvolging" staat op paused; bij heractiveren eerst de welkomstmail-node eruit |
...
| `rebuild.js` | **Edge**, JWT | Triggert een Vercel-deploy via `VERCEL_DEPLOY_HOOK_URL` (herbouwt de statische blog). Admin-knop "Site nu herbouwen" + gekoppeld aan "Nu live"/opslaan (gedeelde `triggerRebuild()`). Edge wegens 12-functielimiet |
| `publish-blog-social.js` | **Edge**, JWT | "Publiceer & deel" (16-07): zet een blogpost live (`published_at`) + deelt hero+caption naar LinkedIn/Instagram via Blotato (hergebruikt de BLOTATO_*-env van `publish-social-post.js`) + triggert de deploy-hook. Instagram vereist een cover_url. Edge wegens 12-functielimiet |
...
| `blog_posts_admin_crud.sql` | Blog-CRUD-policies op stolkwebdesign_blog_posts: authenticated insert/update/delete + auth read-all (admin schrijft/bewerkt/deelt/verwijdert), publiek leest alleen live. Via Management API live 16-07-2026 |
...
**Blog volledig beheerbaar vanuit de admin (16-07):** de Blog-tab is nu volledige CRUD i.p.v. alleen plannen. **`＋ Nieuwe blog` / `✎ Bewerken` / `Verwijderen`** via een modal (titel/slug/onderwerp/samenvatting/inhoud-markdown/hero-upload); `slugify()` leidt de slug af uit de titel (accent-strip). **`Nu live` en opslaan zijn één-kliks:** ze triggeren meteen `api/rebuild` (gedeelde `triggerRebuild()`), met nette terugval als `VERCEL_DEPLOY_HOOK_URL` nog niet is gezet. **`📣 Deel`** opent een deel-modal (kanalen LinkedIn/Instagram + voorgevulde caption + hero-waarschuwing) → `api/publish-blog-social` (Edge): blog live + Blotato. RLS insert/delete toegevoegd (`migrations/blog_posts_admin_crud.sql`). Templates verfijnd brutalist + **8 warme AI-hero's** als `cover_url` (Gemini Flash Image → `sips`-JPEG 275-475KB → Storage `stolkwebdesign-content/blog-heroes/`). Geverifieerd met 3 Puppeteer-gedragstests op de echte admin (36/36 groen) + layout-check. **Bewust nog niet:** inplannen voor later (pg_cron zoals Stolksupport). Zie `docs/logs/2026-07-16/03-…`.
...
- `RESEND_API_KEY` — **staat sinds 21-08 als production-env** (speed-to-lead bevestigingsmail in `lead.js`; zelfde Resend-account als de automations, domein geverifieerd). Bezorging ondertekenen staat nog op kopieer-link
...
- **Leads → eigen CMS** (NIET meer Notion): `api/lead.js` → `stolkwebdesign_client_projects` status `nieuwe_lead` (Kanban-kolom vooraan, tab **Projecten** via `site/admin-klantprojecten.js`) + **Telegram-seintje** (env `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`, hergebruikt de Meta Ads-bot 8646952091 → chat 7677255940). Stap-2 mockup-intake stuurt sinds 10-07 een eigen Telegram-bericht (alle info op één plek).
- **Geparkeerd idee — auto bouw-brief bij een lead:** volledig ontwerp voor een `🚀 Start opzet`-Telegram-knop → Firecrawl-scrape + Anthropic-brief op de lead-kaart staat in `docs/specs/2026-07-10-lead-auto-brief-design.md`. Bewust NIET gebouwd (te veel infra voor het lead-volume); vervangen door de op-aanvraag-route "maak een bouw-brief voor lead #X" in Claude Code. Oppakken als het lead-volume groeit.
- **Outreach-kliks in de Projecten-tab** (10-07): tabel `stolkwebdesign_prospect_clicks` (RLS authenticated read) wordt dagelijks 09:10 gevuld door `scripts/sync-prospect-clicks.mjs` in de monorepo (bron: go-tracking-links in project ncfr…, bots uitgefilterd); `admin-klantprojecten.js` toont per prospect kliks per link (focus-paneel) + totaal (grid-kaart), match op projectnaam. De Booij-opvolgflow gebruikt de admin-status bovendien als stopknop (status ≠ `nieuwe_lead` = contact geweest), zie `Projecten/Booij Hoveniers/CLAUDE.md`.
- **Meta-campagne GESTOPT op 21-08-2026** (account `act_2864094210541845`). Campagne en beide adsets op PAUSED, niets is verwijderd. Eindstand over zeven weken: **€234,99 uitgegeven, 4 leads, 0 klanten.** Alle 4 de leads kwamen uit de oude juli-opzet; de omgezette campagne (07-08 t/m 21-08, maandmodel + `LANDING_PAGE_VIEWS`) leverde 406 landingspagina-weergaven en **nul** leads op. Diagnose: optimaliseren op landingspagina-weergaven maakte het verkeer elf keer goedkoper (€0,17 per bezoeker tegen €1,97) door het budget te verschuiven naar Reels-overlays van €1,97 CPM, wég van Facebook feed, de enige plaatsing die ooit een lead opleverde (juli €58 → 1 lead, augustus €2,48 → 0). Uitgesloten als oorzaak: het formulier werkt (live getest, 200 OK), de pagina heeft geen console-fouten, en beide paginavarianten (mét en zonder prijs in de kop) gaven exact nul. **Bij een eventuele herstart:** terug naar leads als doel, alleen Facebook feed + Instagram feed, en een budget waar Meta iets mee kan; op €5/dag is een product van €1.400-4.200 niet eerlijk te testen. Historie hieronder blijft staan als naslag.
- Oorspronkelijke opzet (juli, historisch): €5/dag, optimaliseerde op Lead-pixel `384832226157615`: opgezet via `Skills/Meta Ads/launch_stolkwebdesign.py` (preset `mockup` → landingspagina). Eigen brutalist-ads (`Skills/Meta Ads/ad-designs/`, HTML→PNG). FB-omslag `marketing/fb-cover-stolkwebdesign.png`. Zie `docs/logs/2026-07-05/01-…` + `02-…`.
  - **Targeting aangescherpt 16-07** (diagnose "veel klikken, weinig leads": LP is prima, verkeer was te breed — bewijs: backend heeft echt 2 ad-leads, klik→lead ≈1,7% is normaal voor koud/breed): ad-set `120246782265430393` van heel-NL-breed → **heel NL + ondernemers/bedrijfseigenaren** (flexible_spec: gedrag Eigenaren kleine bedrijven / Beheerders bedrijfs- & IG-business-pagina's + interesse Ondernemerschap/Kleinbedrijf/Creatief ondernemerschap). Reach ~2,1-2,5M. Oude targeting geback-upt in `Skills/Meta Ads/backups/`. Terugkomactie (nul-meting + check → Telegram) in `Skills/Meta Ads/ads-check/` (`baseline.json` + `check.py` + eenmalige launchd-plist ma 21-07 09:30, activeert Peter zelf). Diagnose/aanpak via de Meta Graph API in `Skills/Meta Ads/scripts/`. Zie `docs/logs/2026-07-16/01-…`.
  - **Het optimalisatiedoel bepaalt wélk verkeer je koopt (15-08, harde les).** Op 07-08 ging het doel van `LEAD` naar `LANDING_PAGE_VIEWS` omdat de leadpixel permanent in de leerfase hing. Gevolg: Meta zoekt sindsdien niet meer naar waarschijnlijke converteerders maar naar de goedkoopste pagina-laders, en dat zijn mensen die op hun telefoon door video scrollen. Gemeten: juli feed 48% van de kliks / 65% van het budget → 4 leads; augustus Reels+Stories+overlay **79,5% van de kliks / 80,3% van het budget → 0 leads**, met `facebook_reels_overlay` als grootste post (82 kliks, 31%, kwam in juli niet voor). CPC zakte van €0,35 naar €0,12, en juist dát was het alarm: drie keer goedkoper verkeer is drie keer minder intentie. Apparaten: 219 iPhone, 43 Android, **0 desktop**. De adset had geen enkele positiebeperking, dus niets hield het tegen. Fix 15-08: `facebook_positions:["feed"]` + `instagram_positions:["stream"]` via `Skills/Meta Ads/scripts/alleen_feeds.py` (dry-run, back-up in `backups/`, `--herstel` om terug te draaien). **Audience Network eruit halen (07-08) loste dit niet op:** het budget verhuisde simpelweg naar de volgende rommelplaatsing, want het dóél nodigt de rommel uit, niet de plaatsingslijst.
  - **Verander nooit prijs, doel en plaatsingen tegelijk** (07-08 én 13-08 misgegaan). Op 07-08 gingen alle drie in één keer om; op 13-08 concludeerde ik uit een juli-tegen-augustus-vergelijking dat de prijs de leads doodde (p=0,034), terwijl in diezelfde periodes óók het doel en de plaatsingsmix veranderden. Die conclusie was dus niet houdbaar: de plaatsingsverschuiving is veel groter en beter onderbouwd. Bij een A/B op één account: één variabele per keer, en reken op ~400 kliks (bij dit volume 2 weken) voor je iets mag concluderen.
  - **Prijs uit kop en H1 (13-08).** Binnen één week haalde de enige advertentie zónder bedrag in kop/ondertitel 3,48% CTR tegen 1,21% en 1,87%. Het bedrag stond óók als H1 op de landingspagina (202 weergaven, 0 formulieren). Kop en ondertitel zijn schoongemaakt (`scripts/prijs_uit_koppen.py`), de H1 draagt nu de belofte van de best presterende advertentie ("Alles voor je website in één bedrag per maand") en het formulier heeft een verplichte budgetvraag die meekomt op de lead-kaart. Het bedrag blijft in de prijskaart en de FAQ. **Bewust niet gedaan:** de beelden opnieuw renderen, want ook in het beeld van de winnaar staat "€350 p/m" en die wint, dus het beeld is niet de rem. Let op bij nieuwe creatives: `degrees_of_freedom_spec.standard_enhancements` is uitgefaseerd (`error_subcode 3858504`) en laat de creative weigeren.
...
- `docs/logs/2026-07-21/03-…` Factuur-module uitgebouwd: print-fix (verkeerd formaat/lege pagina's), PDF-naam=nummer, merk-mail uit Design System, overzichtspagina + offerte/factuur + betaald-stempel
